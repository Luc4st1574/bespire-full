...
const Dropdown: FC<{ trigger: ReactNode; children: ReactNode; menuWidth?: string }> = ({ trigger, children, menuWidth = "w-auto" }) => {
...
export const ZoomControl: FC<ZoomControlProps> = ({ zoom, onZoomChange }) => {
    const zoomOptions = ['50%', '75%', '90%', '100%', '125%', '150%'];

    return (
        <Dropdown trigger={
            <div className="flex items-center p-1.5 px-3 rounded-full bg-gray-100 cursor-pointer text-sm text-[#697d67]">
                <span>{zoom}</span> <ChevronDown size={16} className="ml-1" />
            </div>
        }>
            {zoomOptions.map(o => <DropdownMenuItem key={o} onClick={() => onZoomChange(o)}>{o}</DropdownMenuItem>)}
        </Dropdown>
    );
};